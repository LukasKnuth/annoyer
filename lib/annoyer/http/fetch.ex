defmodule Annoyer.Http.Fetch do
    ...
    def handle_info(_args, state) do
      {:noreply, state} # handle any unknown messages so we dont crash
    end

    def fetch_get(url) do
      case HTTPoison.get(url) do
        {:ok, %HTTPoison.Response{status_code: code, body: body}} when code >= 200 and code < 300 -> {:ok, body}
        {:ok, %HTTPoison.Response{status_code: code}} -> {:error, %HTTPoison.Error{reason: "Unsuccessful Status code #{code} returned."}}
    ...
    def process_interval(%{url: url, body_type: :json, path: path} = config, state) do
      ...
      case delivery do
        :unknown ->
          ...
          Map.delete(state, url) # Nobody cares about this. Remove it.
        :ok -> # Delivered. Reschedule!
          schedule_interval(config)
          state
        {:error, error} -> # Either we we're successful, or there was an error _this time_ and we'd like to retry. todo implement error count and exponential back-off!
          Logger.error("Error processing fetch on topic #{config["topic"]} for #{url}. Rescheduling...", error)
          schedule_interval(config)
          state
      end
    end
